...
knitr::opts_chunk$set(
  ...
  fig.width = 7,
  fig.height = 5,
  eval = FALSE  # Disable evaluation by default to prevent crashes during build
)
```
...
plot(las_solar, color = "solar_potential", bg = "white", size=5)
```
...
opt_chunk_size(ctg) <- 100  # 100m chunks
...
solar_results <- catalog_apply(ctg, function(chunk) {
  ...
  # Extract geographic info from CRS (required for solar calculations)
  crs_info <- vostokR:::extract_crs_info(las)
  
  # Convert date strings to day numbers
  date_range <- vostokR:::date_to_day_numbers('2025-06-15', '2025-06-17', 2025)
  ...
  calculate_solar_potential(
    ...
    minute_step = 60,
    voxel_size = 2.0,
    lat = crs_info$lat,           # Explicitly provide from CRS
    lon = crs_info$lon,           # Explicitly provide from CRS
    timezone = crs_info$timezone  # Explicitly provide from CRS
  )
})
```

Visualize the catalog processing results:

```{r eval=FALSE}
# Merge the list of las into a single las
# Remove NULL results from empty chunks
valid_results <- solar_results[!sapply(solar_results, is.null)]

# Mosaic all chunks into a single raster
solar_mosaic <- do.call(rbind, valid_results)

# Create a simple visualization to validate results
plot(solar_mosaic, color = "solar_potential", bg = "white", size=2)
```
...
available_cores <- parallelly::availableCores(logical = FALSE)
